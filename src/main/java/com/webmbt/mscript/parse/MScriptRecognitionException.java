...
/**
 * Thrown by the {@link com.webmbt.mscript.parse.MScriptParser MScript parser} when (MScript-) specific parse errors
 * occur (e.g. a function call is successfully matched but either the function is not defined or the number of arguments
...
public class MScriptRecognitionException extends RecognitionException {
     ...
     */
    public MScriptRecognitionException(String message, Parser recognizer, Token offendingToken) {
        super(message, recognizer, recognizer.getInputStream(), recognizer.getContext());
        setOffendingToken(recognizer, offendingToken);
    }

    public MScriptRecognitionException(String message, Parser recognizer) {
